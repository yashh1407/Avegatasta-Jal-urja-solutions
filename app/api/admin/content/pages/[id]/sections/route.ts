...
export async function POST(req: Request, { params }: { params: Promise<{ id: string }> }) {
  ...
  try {
    ...
    // Get max sort_order
    const [{ max_sort }] = await query(`SELECT MAX(sort_order) as max_sort FROM page_sections WHERE page_id = ?`, [id]) as any[];
    ...
    const result = await query(
      `INSERT INTO page_sections (page_id, section_type, section_key, category, title, subtitle, content, data_json, sort_order, is_active) 
       VALUES (?, ?, ?, ?, ?, ?, ?, ?, ?, ?)`,
      [
        id, section_type, section_key, category || '', title || '', subtitle || '', content || '',
